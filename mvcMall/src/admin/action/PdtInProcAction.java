...
public class PdtInProcAction implements action.Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String uploadPath = "D:/khs/jsp/work/mvcMall/WebContent/product/pdt_img";
		// 파일을 저장할 실제 위치를 구함
		...
		MultipartRequest multi = new MultipartRequest(
			...
			maxSize, // 한번에 업로드할수있는 최대 크기 (byte단위)
			"utf-8", //파일의 인코딩 방식(utf-8, euc-kr, ksc-5601 등)
			new DefaultFileRenamePolicy()); // 파일 이름의 중복처리
		...
		while ( files.hasMoreElements()) {	// hasMoreElements 다음요소가 있는 지확인
		...
		pdt.setPl_orig(orig);		pdt.setPl_name(name);
		pdt.setPl_opt(opt);			pdt.setPl_view(view);
		pdt.setPl_img1(img1);		pdt.setPl_img2(img2);
		pdt.setPl_img3(img3);		pdt.setPl_desc(desc);
		// 등록할 상품정보를 PdtInfo형 인스턴스 pdt에 담음
		
		boolean isSuccess = pdtInProcSvc.pdtInsert(pdt);
		if (!isSuccess) {	// 상품 등록에 실패했으면
