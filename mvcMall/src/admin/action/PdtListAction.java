...
public class PdtListAction implements action.Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 상품 목록을 저장할 ArrayList객체로 FreeInfo형 인스턴스만 저장함

		request.setCharacterEncoding("utf-8");
		int cpage = 1, pcnt, spage, epage, rcnt, bsize = 10, psize = 10;
		// 페이징에 필요한 값들을 저장할 변수선언 및 초기화
		...
		schtype	= request.getParameter("schtype");	// 검색조건으로 상품 아이디(id), 상품명(name)
		...
		sprice	= request.getParameter("sprice");	// 가격대의 시작 가격대
		eprice	= request.getParameter("eprice");	// 가격대의 종료 가격대
		stock	= request.getParameter("stock");	// 재고량(이상)

		// 정렬 조건 : 가격price(오a내d) 상품명name(오a) 등록일date (오a내d) 인기salecnt(내d) 리뷰review(내d)
		String ord = request.getParameter("ord");
		
		String where="", orderby ="";	// 쿼리 작업시 사용할 조건을 저장할 변수
		...
		if (stock != null && !stock.equals("")) { 
			...
			// 재고량이 무한대이거나 0이면 해당 상품만을 검색하고
			else	where +=" and a.pl_stock >= '"+ stock +"' ";
			// 재고량이 일반 숫자일 경우 해당 숫자 이상인상품들을 검색(무한대 제외)
		}
		// 검색 조건용 where절 생성
		...
		// 상품 목록을 위한 비즈니스 로직을 처리하기 위해 FreeListSvc 인스턴스 생성
		...
		pageInfo.setEdate(edate);		// 등록일 검색종료일
		...
		// 상품목록 화면(pdt_list.jsp)으로 목록(pdtList)과 페이징정보(pageInfo)를 request에 담아 가져감
